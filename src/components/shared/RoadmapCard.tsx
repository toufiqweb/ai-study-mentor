...
export default function RoadmapCard({ roadmap }: { roadmap: RoadmapSummary }) {
  ...
  return (
    <div className="flex h-full flex-col rounded-3xl border border-(--border-subtle) bg-(--surface) p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl">
      <div className="relative h-40 w-full overflow-hidden rounded-2xl">
        ...
        />
        <div className="absolute top-3 left-3 rounded-full bg-(--surface)/90 backdrop-blur-xs p-1.5 shadow-xs">
      ...
      <h3 className="mt-1 text-base font-bold text-(--text-strong)">
        {roadmap.title}
      </h3>
      ...
      <p className="mt-2 text-xs text-(--text-subtle)">By {roadmap.createdBy}</p>

      <Link
        href={`/explore-roadmaps/${roadmap._id}`}
        className="mt-5 rounded-xl bg-(--action-bg) px-4 py-2.5 text-center text-sm font-bold text-(--action-text) transition-colors hover:bg-(--action-hover)"
      >
        View Details
      </Link>
    </div>
